  ...
  <aside class="user-reviews">
    <header class="user-reviews__header">
      <h1 class="user-reviews__h1">
        <h2 class="user-reviews__h2">REVIEW ({{ '433' }})</h2>
      ...
      <div class="user-reviews__point-index">리뷰 작성 시 최대 5,000 포인트 적립!</div>
    </header>

    <div class="user-reviews__carousel">
      <button class="user-reviews__prev">
        <i class="fas fa-chevron-left" />
      </button>
      <ol class="user-reviews__ol">
        <li v-for="(item, index) in inspectedItems" :key="item.id" class="user-reviews__li">
          <div class="user-reviews__wrapper">
            <div class="user-reviews__mask" />
            <img :src="item.path" :alt="`user-reviewsImage${index + 1}`" class="user-reviews__img" />
          </div>
        </li>
      </ol>
      <button class="user-reviews__next">
        <i class="fas fa-chevron-right" />
      </button>
    </div>
    <ol>
      <!-- <review-list v-for :key /> -->
    </ol>
  </aside>
</template>
...
.user-reviews__prev, .user-reviews__next {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 36px;
